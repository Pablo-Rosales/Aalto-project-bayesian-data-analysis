...
6. **Gaussian Model**, done with the three best variables obtained from applying the Multivariate model in order to compare the results of applying it to the dataset and check if it performs better with the selected variables than the considered alternatives. 
...
```{r, echo=FALSE, fig.cap='densities for area worst variable', fig.align='center', fig.pos="H", out.width = '70%'}
knitr::include_graphics('images/area_worst_densities.png')
```

```{r, echo=FALSE, fig.cap='densities for the smoothness mean variable', fig.align='center', fig.pos="H", out.width = '70%'}
knitr::include_graphics('images/smoothness_mean_densities.png')
```

```{r, echo=FALSE, fig.cap='densities for the texture mean variable', fig.align='center', fig.pos="H", out.width = '70%'}
knitr::include_graphics('images/texture_mean_densities.png')
```
...
do.call(grid.arrange, c(p[1:3], ncol=2))
do.call(grid.arrange, c(p[4:6], ncol=2))
```
...
The results for last two variables are not good at all. They improve for the first one. However, they are far from the good results obtained with the other implemented models.
...
Finally, a gaussian model was applied on the three best variables according to the previous method. With this approach, the variable providing the highest accuracy is "area_worst". However, the analysis of the pareto values may suggest that the model was optimistically biased. The obtained accuracies for the other variables are low. In a future and deeper study, the number of chains could be increased when fitting the model, as well as the number of observations, in order to determine if the response of the model improves. Nevertheless, gaussian processes have a high computational cost, so their execution is time consuming.
...
- Gaussian processes require much time to fit the models, hence, for producing this report, R functions for saving the model objects were used.

- The gaussian models are implemented using subsets from the initial data set. Those subsets are randomly generated, therefore, the results and accuracy might slightly vary in future executions.
