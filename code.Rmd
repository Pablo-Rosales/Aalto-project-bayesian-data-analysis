...
# Model definitions
```{r}
model_data <- list(N=length(scaled_cancer_data$diagnosis), y=c(scaled_cancer_data$diagnosis), x=c(scaled_cancer_data$area_mean))
model_stan <- rstan::stan_model(file = "models/model1.stan")
model_sampling <- rstan::sampling(model_stan, data = model_data, refresh=0, seed=SEED)
```

```{r}
monitor(model_sampling)
model_log_lik <- extract_log_lik(model_sampling, parameter_name = "log_lik", merge_chains = FALSE)
...
```{r}
check_hmc_diagnostics(model_sampling)
```
